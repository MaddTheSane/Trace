...
public struct Equation {
	...
	func ABC() -> Vector3D { return Vector3D(A, B, C) }
	func DEF() -> Vector3D { return Vector3D(D, E, F) }
	func GHI() -> Vector3D { return Vector3D(G, H, I) }
}
...
public class Quadric : Shape {
	...
	public func intersectRay(ray: Ray) -> Double {
		...
		let V1 = rRay ⊗ rRay
		let V2 = 2 * Vector3D(rRay.x * rRay.y, rRay.y * rRay.z, rRay.x * rRay.z)
		let V3 = rCam ⊗ rRay
		let V4 = Vector3D(rRay.x * rCam.y + rCam.x * rRay.y, rCam.y * rRay.z + rRay.y * rCam.z, rCam.x * rRay.z + rRay.x * rCam.z)
		let V5 = rRay
		let V6 = rCam ⊗ rCam
		let V7 = 2 * Vector3D(rCam.x * rCam.y, rCam.y * rCam.z, rCam.x * rCam.z)
		let V8 = 2 * rCam;
